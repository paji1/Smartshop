...
$id_category =$_GET['id'];
 ...
 <div class="container-fluid category-page">
  ...
  <form name="sort" action="" method="post" id="Sort" >
  <select name="order" onchange="this.form.submit()">
     <option value="">Sort By</option>
     <option value="Price1" >low to high price</option>
     <option value="price" >hight to low Price</option>
     <option value="name" >name</option>
        ...
        <div class="row">
          ...
          if (!empty($sort)) { // If you Sort it with value of your  select options
                if ($sort == "Price1" ) {
                  $sort = "price";
                    $queryproduct = "SELECT SQL_CALC_FOUND_ROWS id, name, price, id_picture, thumbnail FROM product WHERE id_category = '{$id_category}' ORDER BY $sort  LIMIT {$start}, 16";
                }else {
                    $queryproduct = "SELECT SQL_CALC_FOUND_ROWS id, name, price, id_picture, thumbnail FROM product WHERE id_category = '{$id_category}' ORDER BY $sort DESC LIMIT {$start}, 16";
                }
                    ...
                    <div class="card-image">
                      ...
                      <div class="card-action">
                        <div class="container-fluid">
                          <h5 class="white-text"><?= $price_product; ?> DH</h5>
                        </div>
                      </div>
                  </div>
                </div>
